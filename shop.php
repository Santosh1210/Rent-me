...
<?php include('./includes/navbar.php')?>
    ...
    <main class="main">
        ...
        <div class="site-breadcrumb" style="background: url(assets/img/about/lorbek.png)">
            <div class="container">
                <h2 class="breadcrumb-title">Car Inventory</h2>
                    ...
                    <div class="car-sidebar">
                                ...
                                <ul>
                                    <li>
                                        <div class="form-check">
                                            <input class="form-check-input" type="checkbox" name="brand[]" value="all">
                                            <label class="form-check-label" for="brand"> All Brands</label>
                                        </div>
                                    </li>
                                    <li>
                                        <div class="form-check">
                                            <input class="form-check-input" type="checkbox" id="bmw" name="brand[]" value="BMW">
                                            <label class="form-check-label" for="brand1"> BMW</label>
                                        </div>
                                    </li>
                                    <li>
                                        <div class="form-check">
                                            <input class="form-check-input" type="checkbox" id="toyota" name="brand[]" value="mercedes">
                                            <label class="form-check-label" for="brand2"> Mercedes</label>
                                        </div>
                                    </li>
                                    <li>
                                        <div class="form-check">
                                            <input class="form-check-input" type="checkbox" id="ferrari" name="brand[]" value="ferrari">
                                            <label class="form-check-label" for="brand3"> Ferrari</label>
                                        </div>
                                    </li>
                                    <li>
                                        <div class="form-check">
                                            <input class="form-check-input" type="checkbox" id="audi" name="brand[]" value="audi">
                                            <label class="form-check-label" for="brand4"> Audi</label>
                                        </div>
                                    </li>
                                    
                                </ul>
                                <button type="submit" class="theme-btn" name="car_brand">Apply</button>
                                </form>
                            </div>
                            <div class="car-widget">
                                ...
                                <div class="price-range-box">
                                    <div class="price-range-input">
                                        <input type="text" id="price-amount" name="price_amount" readonly>
                                    </div>
                            ...
                            <div class="car-widget">
                                ...
                                <ul>
                                    <li>
                                        <div class="form-check">
                                            <input class="form-check-input" type="checkbox" id="tran1" value="automatic" name="trans[]">
                                            <label class="form-check-label" for="tran1"> Automatic</label>
                                        </div>
                                    </li>
                                    <li>
                                        <div class="form-check">
                                            <input class="form-check-input" checked="" type="checkbox" id="tran2" value="manual" name="trans[]">
                                            <label class="form-check-label" for="tran2"> Manual</label>
                                        </div>
                                    </li>
                                    <button type="submit" class="theme-btn" name="transmission">Apply</button>
                                </ul>
                                </form>
                            </div>
...
if(isset($_POST['search'])){
    $search = $_POST['searchT'];
      $p_selectquery = "select * from cars where name like '$search%' ";
  }else if(isset($_POST['car_brand'])){
    
    // Check if at least one brand is selected
    if (isset($_POST["brand"]) && !empty($_POST["brand"])) {
        // Sanitize the input to prevent SQL injection
        $selectedBrands = array_map('htmlspecialchars', $_POST["brand"]);

        $p_selectquery = "SELECT * FROM cars WHERE brand IN ('" . implode("', '", $selectedBrands) . "')";
    ...
    if (isset($_POST["trans"]) && !empty($_POST["trans"])) {
        // Sanitize the input to prevent SQL injection
        $selectedTrans = array_map('htmlspecialchars', $_POST["trans"]);

        $p_selectquery = "SELECT * FROM cars WHERE transmission IN ('" . implode("', '", $selectedTrans) . "')";
...
echo $min_amt = isset($amounts[0]) ? intval($amounts[0]) : null;
echo $max_amt = isset($amounts[1]) ? intval($amounts[1]) : null;

    // Now you can use $price_amount as needed
    $p_selectquery = "SELECT * FROM cars WHERE price BETWEEN $min_amt AND $max_amt";

}else if(isset($_POST['sort'])){
    
    if($_POST['sort']==="highest"){
        
         $p_selectquery = "select * from cars order by price desc";
         
    }else{
        $p_selectquery = "select * from cars order by price asc";
